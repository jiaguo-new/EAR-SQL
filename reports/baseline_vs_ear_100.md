...
> ⚠️ VES（Valid Efficiency Score）：当前 `eval/eval_bird.py` 仅实现 EX / Valid / EM / JOIN-EX，
> **未实现 VES**（VES 需 gold 与 pred 的执行耗时比，本仓库 eval 脚本未做）。
> 此处以 **Valid SQL Rate** 作为效率/有效性的近似指标。如需真正的 VES，需在
> `eval_bird.py` 中加入执行耗时统计。这是相对 runbook「EX/VES」要求的一处已知缺口。
...
- **下一步**：
  ...
  2. 在 `eval/eval_bird.py` 加入 VES（执行耗时比）以满足 EX/VES 双指标；
